...
| 名字      | 说明                                                      |
| --------- | --------------------------------------------------------- |
| opcode    | ?积木的类型（例如motion\_gotoxy代表`移动到 x: () y: ()`） |
...
opcode是表示积木的类型，一般由（类型名\_操作名）组成。  
...
| tagName          | 文本"mutation"
| children         | 空数组[]
| hasnext          | (只在`control_stop`中有)能否在后面连接积木（选择`停止该角色的其他脚本`时为`true`） |
| proccode         | (以下只在`procedures_xx`中有)积木定义代码(%s表示文本，%b表示条件，%n表示数字)      |
| argumentids      | ?选择的下拉选项                                           |
| argumentNames    |  是否为可拖出的积木（例如造型选项即为不可拖出的积木）     |
| argumentdefaults |  是否处于最前面(parent等于null)                           |
| warp             |  x坐标（只在最前面的积木出现）                            |
...
1. 自定义积木定义 "procedures\_definition"
